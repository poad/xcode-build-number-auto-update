...
;// CONCATENATED MODULE: ./dist/bundleNumber.js


const get = (infoPlistFile) => {
    if (!external_fs_.statSync(infoPlistFile).isFile()) {
        throw new Error(`${infoPlistFile} is not a file!`);
    }
    return external_child_process_namespaceObject.spawnSync(`/usr/libexec/PlistBuddy -c "Print CFBundleVersion" "${infoPlistFile}"`, {
        cwd: `${process.cwd()}`,
...
const update = (infoPlistFile, bundleNumber) => {
    if (!external_fs_.statSync(infoPlistFile).isFile()) {
        throw new Error(`${infoPlistFile} is not a file!`);
    }
    external_child_process_namespaceObject.execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleVersion ${bundleNumber}" "${infoPlistFile}"`, {
        cwd: `${process.cwd()}`,
...
function run() {
    try {
        ...
        if (autoIncrement !== "true") {
            ...
            const currentNumber = +get(path);
            nextNumber = `${currentNumber + 1}`;
        }
        core.info(`Update to ${nextNumber}`);
        update(path, nextNumber);
